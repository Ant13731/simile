...
\tikzset{%
  % for waterfall diagrams
  previous/.style={below=1cm of #1},
  % Normal tikz shapes
...
\end{figure}
\subsubsection{SSA Lowering}
\subsubsection{LLVM Lowering}
...
Let $\sigma$ represent the symbol table (data store) for assigned variables, defined types, and procedure definitions. Further, let $\bot$ represent an error state. The following sections define small-step semantics for Simile in the form of inference statements from $\langle$Simile code, $\sigma \rangle \to \langle$Derived simile code, $\sigma' \rangle$.
...
\paragraph{Base}
% TODO What do we need to establish a base for these semantics? Error propagation?
\subparagraph{Cast}
\subparagraph{Equals}
...
% \subsection{Language Examples} % TODO move to HLD/motivation
% \section{Features and Behaviours} % TODO move to HLD? This category is too broad
...
% TODO this should go in the specification. Split the abstract rewrites from the concrete ones with a concrete data type lowering section
