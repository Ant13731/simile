...
\begin{figure}
  ...
  \node[stage] (source) {Source program};
  \node[stage, below=of source] (ast) {AST (parse-only)};
  \node[stage, below=of ast] (typed) {AST (post-static analysis) + Symbol Table};
  \node[stage, below=of typed] (rewritten) {AST (post-optimization) + Symbol Table};
  \node[stage, below=of rewritten, minimum height=1.0cm] (concrete) {AST (post-concretization) + concrete Symbol Table};
  \node[stage, below=of concrete, minimum height=1.0cm] (concreteopt) {AST (post-concrete-optimization) + concrete Symbol Table};
  \node[stage, below=of concreteopt] (ssa) {SSA};
  \node[stage, below=of ssa] (llvm) {LLVM IR};
  \node[stage, below=of llvm] (executable) {Executable code};

  \draw[arrow] (source) -- node[right, align=left] {Parse} (ast);
  \draw[arrow] (ast) -- node[right, align=left] {Static analysis} (typed);
  \draw[arrow] (typed) -- node[right, align=left] {General rewrite optimizations} (rewritten);
  \draw[arrow] (rewritten) -- node[right, align=left] {Abstract-to-concrete datatype selection} (concrete);
  \draw[arrow] (concrete) -- node[right, align=left] {Concrete-datatype-specific rewrite optimizations} (concreteopt);
  \draw[arrow] (concreteopt) -- node[right, align=left] {Braun's algorithm} (ssa);
  \draw[arrow] (ssa) -- node[right, align=left] {LLVM lowering} (llvm);
  \draw[arrow] (llvm) -- node[right, align=left] {Compiled by LLVM} (executable);

  \end{tikzpicture}
  \caption{Simile compilation pipeline from source code to LLVM-based executable}
  \label{fig:pipeline}
\end{figure}
% TODO make a diagram for the internal structure of each component (ex. parser has a scanner and recursive-descent parser, static analysis has: symbol table + type synthesis + traits synthesis, optimizer has desugaring + simrw + library. Lets plan this out a bit more
% Then we can create a tree diagram for our data structures - see drawio/convo for tikz format.
...
\begin{gather}
  ...
  \begin{prooftree}
    ...
    \hypo{t: relation[V, U]}
    \hypo{extremum \in \{relationalDomainMin, relationalDomainMin\}}
  ...
  \begin{prooftree}
    ...
    \hypo{extremum \in \{relationalRangeMin, relationalRangeMin\}}
    \infer3{s \circ t: relation[T, U, extremum=m]}
  \end{prooftree}
\end{gather}
...
\begin{gather}
  ...
  \begin{prooftree}
    ...
    \infer1{\{q\}: set[T, literal=l]}
  \end{prooftree}
  \\
  \tag{Domain}
  \begin{prooftree}
    \hypo{q: Quantifier[T, domain=d]}
    \infer1{\{q\}: set[T, domain=d]}
  \end{prooftree}
\end{gather}
\subparagraph{Map}
\begin{gather}
